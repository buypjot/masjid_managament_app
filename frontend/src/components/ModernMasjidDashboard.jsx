    ...
    <div className="relative min-h-screen overflow-hidden bg-[#f5f7fb] font-sans text-slate-900">
      ...
      <main className="relative z-10 mx-auto max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-slate-950 via-indigo-950 to-indigo-800 p-6 text-white shadow-2xl shadow-indigo-950/20 sm:p-8">
          ...
          <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-indigo-100"><Sparkles className="h-3.5 w-3.5 text-cyan-300" />Live workspace</div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">Welcome back, {userName.split(' ')[0]}.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-indigo-100/80">{masjidName} is ready for today. Keep your community, collections and operations moving from one calm workspace.</p></div><div className="flex items-center gap-3"><div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur"><div className="text-[9px] font-bold uppercase tracking-wider text-indigo-200">Today</div><div className="mt-1 flex items-center gap-2 text-xs font-bold"><CalendarDays className="h-4 w-4 text-cyan-300" />{currentDate}</div></div><button onClick={() => setShowCollect(true)} className="inline-flex items-center gap-2 rounded-2xl bg-white px-5 py-3.5 text-xs font-black text-slate-950 shadow-xl transition hover:-translate-y-1 hover:shadow-2xl"><Plus className="h-4 w-4" />Collect Santha</button></div></div>
        </motion.section>
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          ].map((card, index) => { const Icon = card.icon; return <motion.div key={card.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }} className="group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/5"><div className="flex items-start justify-between"><div><div className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">{card.label}</div>{loading ? <div className="mt-3 h-9 w-28 animate-pulse rounded-lg bg-slate-100" /> : <div className="mt-2 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">{card.value}</div>}<div className="mt-1 text-[10px] font-medium text-slate-400">{card.secondary}</div></div><div className={`rounded-2xl p-3 ${card.accent === 'indigo' ? 'bg-indigo-50 text-indigo-600' : card.accent === 'emerald' ? 'bg-emerald-50 text-emerald-600' : card.accent === 'amber' ? 'bg-amber-50 text-amber-600' : 'bg-cyan-50 text-cyan-600'}`}><Icon className="h-5 w-5" /></div></div><div className="mt-4 flex items-center gap-2 text-[10px] font-bold text-slate-400"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{card.caption}</div></motion.div>; })}
        </section>
        ...
        <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.55fr_1fr]">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6">
            <div className="mb-5 flex items-center justify-between"><div><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-slate-400"><BarChart3 className="h-4 w-4 text-indigo-500" />Collection trend</div><h2 className="mt-1 text-lg font-black text-slate-950">Monthly Collection</h2></div><span className="rounded-full bg-indigo-50 px-3 py-1.5 text-[10px] font-black text-indigo-600">12 month view</span></div>
            <div className="flex h-64 items-end gap-2 overflow-hidden px-1 sm:gap-3">{Array.from({ length: 12 }, (_, index) => { const item = monthlyData.find((m) => Number(m.month) === index + 1) || monthlyData[index] || {}; const amount = Number(item.amountValue ?? String(item.amount || '0').replace(/[^0-9.]/g, '')) || 0; const height = amount ? Math.max(10, Math.round((amount / maxMonth) * 100)) : 5; return <div key={index} className="group flex h-full flex-1 flex-col items-center justify-end gap-2"><div className="relative flex w-full flex-1 items-end"><motion.div initial={{ height: 0 }} animate={{ height: `${height}%` }} transition={{ duration: 0.8, delay: index * 0.04 }} className="w-full rounded-t-xl bg-gradient-to-t from-indigo-600 via-violet-500 to-cyan-400 opacity-90 transition group-hover:opacity-100" title={formatCurrency(amount)} /></div><span className="text-[9px] font-bold text-slate-400">{monthNames[index]}</span></div>; })}</div>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6"><div className="mb-4 flex items-center justify-between"><div><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-slate-400"><Activity className="h-4 w-4 text-emerald-500" />Live activity</div><h2 className="mt-1 text-lg font-black text-slate-950">Recent Activity</h2></div><button onClick={() => navigate('/dashboard/collections/santha')} className="text-[10px] font-black text-indigo-600 hover:text-indigo-800">View all</button></div><div className="space-y-2">{loading ? Array.from({ length: 5 }).map((_, i) => <div key={i} className="flex items-center gap-3 rounded-xl p-2"><div className="h-9 w-9 animate-pulse rounded-xl bg-slate-100" /><div className="flex-1"><div className="h-3 w-32 animate-pulse rounded bg-slate-100" /><div className="mt-2 h-2 w-20 animate-pulse rounded bg-slate-100" /></div></div>) : activities.slice(0, 5).map((activity, index) => <div key={activity.id || index} className="flex items-center gap-3 rounded-2xl border border-slate-100 p-3 transition hover:border-indigo-100 hover:bg-indigo-50/30"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600"><ArrowUpRight className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="truncate text-xs font-black text-slate-900">{activity.title || 'Collection'}</div><div className="truncate text-[10px] text-slate-400">{activity.subtitle || 'Registered family'} · {activity.time || 'Recently'}</div></div><div className="text-xs font-black text-emerald-600">+{formatCurrency(activity.amount)}</div></div>)}{!loading && activities.length === 0 && <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center"><Activity className="mx-auto h-8 w-8 text-slate-300" /><div className="mt-2 text-xs font-bold text-slate-500">No activity yet</div><div className="mt-1 text-[10px] text-slate-400">Your live collection activity will appear here.</div></div>}</div></motion.div>
        </section>

        <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_1.4fr]">
          <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6"><div className="mb-4 flex items-center justify-between"><div><div className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Workspace</div><h2 className="mt-1 text-lg font-black text-slate-950">Quick modules</h2></div><SlidersHorizontal className="h-5 w-5 text-slate-300" /></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1">{quickModules.map((module) => { const Icon = module.icon; return <button key={module.title} onClick={() => navigate(module.path)} className={`group flex items-center gap-3 rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow-md ${toneClasses[module.tone]}`}><div className="rounded-xl bg-white/80 p-2.5 shadow-sm"><Icon className="h-4 w-4" /></div><span className="min-w-0 flex-1"><span className="block text-xs font-black">{module.title}</span><span className="mt-0.5 block text-[10px] opacity-70">{module.description}</span></span><ArrowRight className="h-4 w-4 opacity-40 transition group-hover:translate-x-1" /></button>; })}</div></div>
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-violet-600 to-cyan-500 p-6 text-white shadow-xl shadow-indigo-500/20"><div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-white/10 blur-2xl" /><div className="relative flex h-full flex-col justify-between gap-6"><div><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider"><ShieldCheck className="h-3.5 w-3.5" />Secure workspace</div><h2 className="mt-4 max-w-lg text-2xl font-black tracking-tight">Everything your Masjid team needs, in one place.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-white/75">Use the navigation to manage community members, collections, properties, services and reports while keeping your existing live data connected.</p></div><div className="flex flex-wrap gap-2 text-[10px] font-bold"><span className="rounded-full bg-white/10 px-3 py-2">Live API</span><span className="rounded-full bg-white/10 px-3 py-2">Responsive</span><span className="rounded-full bg-white/10 px-3 py-2">Secure account</span></div></div></div>
        </section>
      </main>

      <footer className="relative z-10 border-t border-slate-200/70 bg-white/70 px-4 py-5 text-center text-[10px] font-semibold text-slate-400 backdrop-blur"><span>{masjidName}</span> · Live PostgreSQL Backend Connected</footer>

      {showSearch && <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-950/40 p-4 pt-20 backdrop-blur-sm" onMouseDown={() => setShowSearch(false)}><div className="w-full max-w-2xl rounded-3xl border border-white/40 bg-white p-5 shadow-2xl" onMouseDown={(e) => e.stopPropagation()}><div className="flex items-center gap-3"><Search className="h-5 w-5 text-indigo-500" /><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search dashboard modules..." className="flex-1 bg-transparent text-sm font-bold text-slate-900 outline-none placeholder:text-slate-400" /><button onClick={() => setShowSearch(false)}><X className="h-5 w-5 text-slate-400" /></button></div><div className="mt-4 space-y-2">{filteredModules.map((module) => <button key={module.title} onClick={() => { setShowSearch(false); navigate(module.path); }} className="flex w-full items-center gap-3 rounded-2xl border border-slate-100 p-3 text-left hover:bg-slate-50"><module.icon className="h-5 w-5 text-indigo-500" /><span className="flex-1"><span className="block text-xs font-black">{module.title}</span><span className="text-[10px] text-slate-400">{module.description}</span></span><ArrowRight className="h-4 w-4 text-slate-300" /></button>)}{filteredModules.length === 0 && <div className="py-6 text-center text-xs text-slate-400">No matching modules.</div>}</div></div></div>}

    ...
    <div className="mds-reference-dashboard relative min-h-screen overflow-hidden bg-[#f5f7fb] font-sans text-slate-900">
      ...
      <main className="relative z-10 mx-auto max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mds-hero relative overflow-hidden rounded-[28px] p-6 text-white shadow-2xl sm:p-8">
          ...
          <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em] text-indigo-100"><Sparkles className="h-4 w-4 text-amber-200" />Live workspace</div><div className="mds-hero-secure"><ShieldCheck className="h-3.5 w-3.5" />Secure Workspace</div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">Welcome back, {userName.split(' ')[0]}.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-indigo-100/80">{masjidName} is ready for today. Keep your community, collections and operations moving from one calm workspace.</p><div className="mds-hero-trust"><ShieldCheck className="h-3.5 w-3.5" /> Secure workspace connected to your live data.</div></div><div className="flex items-center gap-3"><div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur"><div className="text-xs font-extrabold uppercase tracking-wider text-indigo-200">Today</div><div className="mt-1 flex items-center gap-2 text-xs sm:text-sm font-bold"><CalendarDays className="h-4 w-4 text-cyan-300" />{currentDate}</div></div><button onClick={() => setShowCollect(true)} className="inline-flex items-center gap-2 rounded-2xl bg-white px-5 py-3.5 text-xs sm:text-sm font-black text-slate-950 shadow-xl transition hover:-translate-y-1 hover:shadow-2xl"><Plus className="h-4 w-4" />Collect Santha</button></div></div>
        </motion.section>

...
def get_properties(search: Optional[str] = None, db: Session = Depends(get_db)):
...
def create_property(prop_in: PropertyCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(Property).filter(Property.property_number == prop_in.property_number).first()
    if existing:
        count = db.query(Property).count()
...
def create_tenant(tenant_in: TenantCreate, db: Session = Depends(get_db)):
    ...
    if tenant_in.property_id:
        prop = db.query(Property).filter(Property.id == tenant_in.property_id).first()
        if prop:
...
def get_rent_collections(db: Session = Depends(get_db)):
    """Fetch rent collection history."""
    return db.query(RentCollection).order_by(RentCollection.id.desc()).all()


@router.post("/collections/list", response_model=RentCollectionResponse, status_code=status.HTTP_201_CREATED)
def create_rent_collection(col_in: RentCollectionCreate, db: Session = Depends(get_db)):
    """Record a rent collection payment."""
    if not col_in.receipt_no:
        count = db.query(RentCollection).count()
        col_in.receipt_no = f"RCP-2026-{count + 101:03d}"

    db_col = RentCollection(**col_in.dict())
    db.add(db_col)
    db.commit()
    db.refresh(db_col)
    return db_col
...
def get_hall_bookings(db: Session = Depends(get_db)):
    """Fetch hall bookings."""
    return db.query(HallBooking).order_by(HallBooking.id.desc()).all()


@router.post("/hall-bookings/list", response_model=HallBookingResponse, status_code=status.HTTP_201_CREATED)
def create_hall_booking(hb_in: HallBookingCreate, db: Session = Depends(get_db)):
    """Create a hall booking."""
    if not hb_in.booking_id:
        count = db.query(HallBooking).count()
        hb_in.booking_id = f"HB-{count + 901:03d}"

    db_hb = HallBooking(**hb_in.dict())
    db.add(db_hb)
    db.commit()
    db.refresh(db_hb)
    return db_hb
...
def get_cooking_vessels(db: Session = Depends(get_db)):
    """Fetch cooking vessels inventory."""
    return db.query(CookingVessel).order_by(CookingVessel.id.desc()).all()


@router.post("/vessels/list", response_model=CookingVesselResponse, status_code=status.HTTP_201_CREATED)
def create_cooking_vessel(cv_in: CookingVesselCreate, db: Session = Depends(get_db)):
    """Add cooking vessel to inventory."""
    if not cv_in.vessel_id:
        count = db.query(CookingVessel).count()
        cv_in.vessel_id = f"CVS-{count + 1:02d}"

    db_cv = CookingVessel(**cv_in.dict())
    db.add(db_cv)
    db.commit()
    db.refresh(db_cv)
    return db_cv
...
def get_property_documents(db: Session = Depends(get_db)):
    """Fetch property documents."""
    return db.query(PropertyDocument).order_by(PropertyDocument.id.desc()).all()


@router.post("/documents/list", response_model=PropertyDocumentResponse, status_code=status.HTTP_201_CREATED)
def create_property_document(doc_in: PropertyDocumentCreate, db: Session = Depends(get_db)):
    """Add a property document record."""
    if not doc_in.doc_id:
        count = db.query(PropertyDocument).count()
        doc_in.doc_id = f"DOC-{count + 1:02d}"

    db_doc = PropertyDocument(**doc_in.dict())
    db.add(db_doc)
    db.commit()
    db.refresh(db_doc)
    return db_doc
